...
import com.ditclear.paonet.FactoryProvider
...
abstract class BaseActivity<VB : ViewDataBinding> : AppCompatActivity(), Presenter {
    ...
    val factory: ViewModelProvider.Factory by lazy {
        if (application is FactoryProvider) {
            val mainApplication = application as FactoryProvider
            return@lazy mainApplication.getViewModelFactory()
        } else {
            throw IllegalStateException("application has not implement FactoryProvider")
        }
    }
